<template>
	<view class="">
		...
		>
			<view style="position: relative;min-height: 100rpx;"
			...
			>
				<view class="u-steps__item">
					...
					<text class="u-line-1" v-if="item.name" :style="{
						color: index <= current ? activeColor : unActiveColor,
					}" :class="['u-steps__item__text--' + direction]">
						{{ item.name }}
					</text>
					<text class="u-line-1" style="color:#98A1A9;font-size:20rpx" :class="['u-steps__item__text--' + direction]">
						{{ item.time }}
					</text>
				</view>
				...
				<view class="u-steps__item__type">
					{{ item.type }}
				</view>
			</view>
		</view>
