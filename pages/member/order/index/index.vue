<template>
    <view>
		...
		:is-back="true"
		:is-fixed="true"
		title="订单列表" :background="background" :title-color="titleColor"></u-navbar>
            ...
            <view class="page-box" v-if="listData.length > 0">
                <view class="orderList" v-for="(order, orderIndex) in listData" :key="orderIndex">
                    <view class="top" @click="goOrderDetail(order.orderId)">
                        ...
                        <view class="right"
						:class="order.orderStatus">{{ order.orderStatusName }}</view>
                    </view>
                    <view class="item" v-for="(goods, indexGoods) in order.items" :key="indexGoods">
						<view class="top">
							<view class="titles u-line-2">{{goods.name}}</view>
							<view class="price">
								￥{{ goods.price }}
							</view>
						</view>
                        <view class="center">
							<view class="image">
								<image :src="goods.imageUrl && goods.imageUrl!='null' ?  goods.imageUrl : '/static/images/common/empty-banner.png'" mode="aspectFill"></image>
							</view>
							<view class="content">
								<view class="total">
								...
								<view class="total">
									合计:<text>￥{{ order.orderAmount }}</text>
								</view>
                ...
                <u-loadmore :status="loadStatus" bgColor=""></u-loadmore>
            </view>
                    ...
                    switch (item.status) {
                        case 1:
                            if (item.payStatus === 1) {
                                ...
                                this.$set(item, 'orderStatus', 'paying_order');
                            } else if (item.payStatus >= 2 && item.shipStatus === 1) {
                                this.$set(item, 'orderStatusName', '待发货');
                                this.$set(item, 'orderStatus', 'finish_order');
                        ...
                        case 3:
                                ...
                                this.$set(item, 'orderStatus', 'cancel_order');
                            break
                    }
                });
