<template>
    <view>
		...
		:is-back="true"
		:is-fixed="true"
		title="订单详情" :background="background" :title-color="titleColor"></u-navbar>
        <!--步骤条区域-->
        <view class="u-padding-20" v-if="basics < 9">
            <!--提示-->
            <view class="u-font-sm u-text-center u-margin-top-30" v-if="basics == 0">
                <view class="coreshop-text-white">拍下成功，待买家支付。</view>
                <view class="coreshop-text-white">
            ...
            <view class="u-font-sm u-text-center u-margin-top-30" v-if="basics == 1">
                <view class="coreshop-text-white">支付成功，待卖家发货.</view>
            </view>
            <view class="u-font-sm u-text-center u-margin-top-30" v-if="basics == 2">
                <view class="coreshop-text-white">已发货，快递正在路上，务必在收到商品后再确认收货。</view>
                <view class="coreshop-text-white">
            ...
            <view class="u-font-sm u-text-center u-margin-top-30" v-if="basics == 3">
                <view class="coreshop-text-white">已收货，请您对此次购物体检进行评价。</view>
                <view class="coreshop-text-white">
            ...
            <!--状态图标-->
            <view class="u-padding-20 u-text-center coreshop-status-img-view u-margin-top-20" v-if="basics == 4">
                <view class="are-img-view">
                    <image class="are-img" src="/static/images/common/are.png" mode="widthFix" />
                </view>
                <view class="u-font-sm coreshop-text-white">交易成功，感谢您的评价</view>
            </view>
            <!--状态图标-->
            <view class="u-padding-20 u-text-center coreshop-status-img-view u-margin-top-20" v-if="basics == 6">
                <view class="are-img-view">
                    <image class="are-img" src="/static/images/common/are.png" mode="widthFix" />
                </view>
                <view class="u-font-sm coreshop-text-white">交易成功.期待下次服务。</view>
            </view>
            <!--状态图标-->
            <view class=" u-padding-20 u-text-center coreshop-status-img-view u-margin-top-20" v-if="basics == 7">
                <view class="are-img-view">
                    <image class="are-img" src="/static/images/common/arg.png" mode="widthFix" />
                </view>
                <view class="u-font-sm coreshop-text-white">订单已取消</view>
            </view>
        </view>
        <!--商品信息-->
        <view class=" coreshop-card-box">
            <view class="coreshop-card-view coreshop-shop-view">
                <view class="goods-list-view" v-for="item in orderInfo.items" :key="item.id">
					<view class="coreshop-top">
						<view class="coreshop-text-white u-line-2 coreshop-title" @click="goGoodsDetail(item.goodsId)">{{ item.name }}</view>
						<view class="coreshop-text-white coreshop-text-price u-font-lg">{{ item.price }}</view>
					</view>
                    <view class="goods-info-view">
						<image class="coreshop-avatar radius" :src="item.imageUrl" mode="aspectFill"></image>
						<view class="coreshop-info">
							<view class="coreshop-text-white u-line-1">
								<view class="coreshop-text-white title-view" v-if="orderInfo.goodsDiscountAmount > 0">
								    <view class="title">优惠信息：商品优惠</view>
								    <view class="u-text-right">
								        <text class="u-margin-right-20">-</text>
								        <text class="coreshop-text-price coreshop-text-blue">{{ orderInfo.goodsDiscountAmount }}</text>
								    </view>
								</view>
								<view class="coreshop-text-white title-view" v-if="orderInfo.pointMoney > 0">
								    <view class="title">优惠信息：积分优惠</view>
								    <view class="u-text-right">
								        <text class="u-margin-right-20">-</text>
								        <text class="coreshop-text-price coreshop-text-blue">{{ orderInfo.pointMoney }}</text>
								    </view>
								</view>
								<view class="coreshop-text-white title-view" v-if="orderInfo.orderDiscountAmount > 0">
								    <view class="title">优惠信息：订单优惠</view>
								    <view class="u-text-right">
								        <text class="u-margin-right-20">-</text>
								        <text class="coreshop-text-price coreshop-text-blue">{{ orderInfo.orderDiscountAmount }}</text>
								    </view>
								</view>
								<view class="coreshop-text-white title-view" v-if="orderInfo.couponDiscountAmount > 0">
								    <view class="title">优惠信息：优惠券优惠</view>
								    <view class="u-text-right">
								        <text class="u-margin-right-20">-</text>
								        <text class="coreshop-text-price coreshop-text-blue">{{ orderInfo.couponDiscountAmount }}</text>
								    </view>
								</view>
							</view>
							<view class="coreshop-text-white u-font-sm coreshop-paid">
								已支付：<text class="coreshop-text-blue">¥{{ orderInfo.goodsAmount}}</text>
							</view>
                       </view>
                    </view>
                </view>
        ...
        <!-- 团购分享拼单 -->
        <view class=" coreshop-card-box" v-if="orderInfo.orderType==2 && orderInfo.status != 3 && orderInfo.payStatus!=1">
            <view class="coreshop-card-view coreshop-address-view">
                <view v-if="teamInfo.status==1" class="u-font-lg coreshop-text-bold coreshop-text-white">待拼团，还差{{ teamInfo.teamNums || ''}}人</view>
                <view v-else-if="teamInfo.status==2" class="u-font-lg coreshop-text-bold coreshop-text-white">拼团成功，待发货</view>
                <view v-else-if="teamInfo.status==3" class="u-font-lg coreshop-text-bold coreshop-text-white">拼团失败</view>
        ...
        <view class=" coreshop-card-box">
            <view class="coreshop-card-view coreshop-order-view">
                <view class="coreshop-text-white title-view">
               ...
               <view class="coreshop-text-white title-view">
                   ...
                   <view class="u-text-right">
                       <text>
                           {{ orderInfo.globalStatusText || ''}}
                       </text>
                   </view>
               </view>
			   <view class="coreshop-text-white title-view" v-if="orderInfo.createTime">
			       <view class="title">下单时间</view>
			       <view class="u-text-right">
			           <text>{{ orderInfo.createTime || ''}}</text>
			       </view>
			   </view>
			   <view class="coreshop-text-white title-view" v-if="basics > 0 && orderInfo.paymentTime">
			       <view class="title">支付时间</view>
			       <view class="u-text-right">
			           <text>{{ orderInfo.paymentTime || ''}}</text>
			       </view>
			   </view>
                <view class="coreshop-text-white title-view" v-if="basics > 1 && delivery && delivery.createTime">
                    <view class="title">发货时间</view>
                    <view class="u-text-right">
                        <text>{{ delivery.createTime || ''}}</text>
                    </view>
                </view>
                <view class="coreshop-text-white title-view" v-if="basics > 2 && orderInfo.confirmTime">
                    <view class="title">确认时间</view>
                    <view class="u-text-right">
                        <text>{{ orderInfo.confirmTime || ''}}</text>
                    </view>
                </view>
			   <view class="coreshop-text-white title-view" v-if="basics > 3 && orderInfo.updateTime">
                ...
                <view class="coreshop-text-white title-view" v-if="orderInfo.paymentName && orderInfo.payStatus > 1">
        ...
        <!--物流信息-->
        <view class="coreshop-card-box" v-if="basics != 0 && !orderInfo.store">
            <view class="coreshop-card-view coreshop-address-view">
                <view class="u-font-lg coreshop-text-bold coreshop-text-white flex justify-between">
                    物流信息
                </view>
				<view class="coreshop-map">
					
				</view>
                <view class="wrap" v-if="basics < 9"> 
					<!--步骤条-->
					<u-steps :list="numList" 
					direction="column"
					active-color="#02B9EE" :current="basics" mode="dot" v-if="basics < 5"></u-steps>
        ...
        <!--提货信息-->
        <view class=" coreshop-card-box" v-if="orderInfo.store">
            <view class="coreshop-card-view coreshop-order-view">
                <view class="u-font-lg coreshop-text-bold coreshop-text-white">提货信息</view>
                <u-line color="#eee" border-style="dashed" margin="20rpx 0" />
                <view class="coreshop-text-white coreshop-text-bold title-left-view u-margin-bottom-20">
                ...
                </view> <view class="coreshop-text-white title-view">
                ...
                <view class="coreshop-text-white title-view" v-if="lading.status">
        ...
        <!--发票信息-->
        <view class="coreshop-card-box" v-if="orderInfo.invoice && orderInfo.invoice.type != 1">
            <view class="coreshop-card-view coreshop-order-view">
                <view class="u-font-lg coreshop-text-bold coreshop-text-white">发票信息</view>
                <u-line color="#eee" border-style="dashed" margin="20rpx 0" />
        ...
        <!-- <view class=" coreshop-card-box">
            <view class="coreshop-card-view coreshop-price-view">
                <view class="u-font-lg coreshop-text-bold coreshop-text-white">费用信息</view>
                <u-line color="#eee" border-style="dashed" margin="20rpx 0" />
                <view class="coreshop-text-white title-view" v-if="orderInfo.promotionObj && orderInfo.promotionObj.length > 0">
                ...
                <view class="coreshop-text-white title-view" v-if="orderInfo.payStatus > 1">
                    <view class="title">支付方式</view>
                    <view class="u-text-right">
                        <text>{{ orderInfo.paymentName }}</text>
                    </view>
                ...
                <view class="coreshop-text-white coreshop-text-bold title-right-view">
      ...
      <!--  <view class="coreshop-recommended-title-view" v-if="otherData.length>0">
            <view class="u-flex u-flex-wrap">
                ...
                <view class="u-flex-4 u-text-center">
                    <text class="coreshop-text-white u-font-lg">为您推荐</text>
                </view>
       ...
       <!-- <view class="coreshop-goods-group" v-if="otherData.length>0">
        ...
        <view class="coreshop-footer-fixed coreshop-foot-padding-bottom u-text-center u-padding-top-10" v-if="orderInfo.status == 1 || orderInfo.status == 2">
    ...
    export default {
        ...
        data() {
            return {
                ...
                numList: [{ name: '运输中',time:'2022-04-23 16:23:43',type:"【北京市】快件到达【北京转运中心】" }, 
				{ name: '',time:'2022-04-23 16:23:43' ,type:"商家已发货"  }, 
				{ name: '' ,time:'2022-04-23 16:23:43' ,type:"付款成功" },
				{ name: '',time:'2022-04-23 16:23:43' ,type:"下单成功"  }],
                basics: 0,
                delivery: {},//发货信息
                orderId: 0,
                orderInfo: {}, // 订单详情
