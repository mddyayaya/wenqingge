<template>
    <view>
        ...
        <u-navbar title="提交订单" background="#61676F" title-color="#fff"></u-navbar>
        <div class="content">
            <view>
                <!-- <view class="u-margin-left-20 u-margin-right-20 u-margin-top-20">
                    <u-subsection :list="typeItems" mode="subsection" :current="typeCurrent" @change="onTypeItem" bg-color="#ffffff" active-color="#e54d42" style="border-radius:0;"></u-subsection>
                </view> -->
                <view class="content">
                    <view v-show="typeCurrent === 0 || typeCurrent === 1">
                        <!-- 收货地址信息 -->
                        <view class="coreshop-common-view-box" v-if="userShip && userShip.id" @click="showAddressList">
							<view class="user_list">
								<view class="coreshop-list-item">
									<view class="buy_type">
										<view :class="{select_types:typeCurrent==0}" @click="onTypeItem(0)">快递</view>
										<view :class="{select_types:typeCurrent==1}" @click="onTypeItem(1)">自提</view>
									</view>
									<view class="cust_item">收货人：{{ userShip.name || '' }}</view>
									<view class="cust_item">{{ userShip.mobile || ''}}</view>
									<view class="cust_item">{{ userShip.areaName || ''}}</view>
								</view>
								<view class="address_more"  @click="goAddress()">
									更改
								</view>
							</view>
                        </view>
                        <view class="u-margin-bottom-20 coreshop-common-view-box" v-else>
                            <view class="coreshop-bg-white coreshop-card address-view u-text-center">
                    ...
                    <view v-show="typeCurrent === 2">
                        <!-- 门店信息 -->
                        <view class="u-margin-bottom-20 coreshop-common-view-box" v-if="store && store.id" @click="goStorelist()">
                ...
                <!-- 商品列表信息 -->
                <view class="u-margin-bottom-20 coreshop-common-view-box">
                    <view class="goods-view">
                        <view class="goods-info-view-box" v-if="item.isSelect == true" 
						v-for="(item, index) in products" :key="index">
							<view class="products_info">
								<view class="coreshop-avatar">
									<image :src="item.products.images" mode="widthFix"></image>
								</view>
								<view class="goods-info-view">
								    <!-- <view class="coreshop-text-white u-line-2">{{ item.products.name || ''}}</view> -->
								    <view class="u-font-sm coreshop-text-white" v-if="item.products.spesDesc !== null">{{ item.products.spesDesc || ''}}</view>
								...
								<view class="goods-num-view">
									<u-number-box size="small" v-model="item.nums"></u-number-box>
									<!-- <text>数量{{ item.nums || ''}}</text> -->
								</view>
							</view>
							<!-- 配送方式 -->
							<view class="goods-send-view">
								<text>配送方式</text>
								<text>{{typeCurrent==0?'快递':'自提'}}</text>
							</view>
							<view class="goods-send-view">
								<text>优惠</text>
								<text>减￥30</text>
							...
							<view class="goods-send-view">
								<text>总价</text>
								<text class="total_price">￥{{ item.products.price || ''}}</text>
							</view>
							<!-- <view class="goods-send-view goods-pay">
								<text>支付方式</text>
								<view class="goods-pay_right">
									<text>微信支付</text>
									<u-icon name="arrow-right" size="20" color="#02B9EE"></u-icon>
								</view>
							</view> -->
                        </view>
                    </view>
           ...
           <!-- <view class="u-margin-top-20 u-margin-bottom-20 coreshop-common-view-box">
            ...
            <!--底部操作-->
            <view class="coreshop-footer-fixed coreshop-foot-padding-bottom u-p-b-30">
				<view class="coreshop-text-white price-view">
					<!-- <text class="u-margin-right-20">共 {{ productNums}} 件商品</text> -->
					<view>待支付合计<text class="money coreshop-text-bold"> ￥{{ cartData.amount}}</text></view>
					<view>共优惠<text class="money">￥30</text></view>
				</view>
				<view class="pay_btn"  @click="toPay" :disabled='submitStatus' :loading='submitStatus'>
					点击支付
				</view>
